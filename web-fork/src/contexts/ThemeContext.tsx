...
export default function ThemeContextProvider(props: Props) {
    ...
    // Mirror the resolved theme onto <html data-theme> so shadcn/Tailwind components (driven by the
    // CSS variables in index.css) reflect the selected theme.
    useEffect(() => {
...
// Resolves a stored theme name (including "auto") to the concrete [data-theme] value used by the
// CSS-variable theme blocks in index.css.
function ResolveThemeName(name: string, prefersDark: boolean): string {
    switch (name) {
        ...
        default:
            return prefersDark ? themes.ThemeNameDark : themes.ThemeNameLight;
    }
}
