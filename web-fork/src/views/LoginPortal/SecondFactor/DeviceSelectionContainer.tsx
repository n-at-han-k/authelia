...
const DefaultDeviceSelectionContainer = function (props: Props) {
    ...
    switch (state) {
        case State.DEVICE:
            container = (
                <div className={"flex flex-wrap justify-center gap-2"} id={"device-selection"}>
        ...
        case State.METHOD:
            container = (
                <div className={"flex flex-wrap justify-center gap-2"} id={"method-selection"}>
    ...
    return (
        <div className={"mx-auto w-full max-w-screen-md px-4"}>
            {container}
            <Button onClick={props.onBack} id={"device-selection-back"}>
                back
            </Button>
        </div>
    );
...
const DeviceItem = function (props: DeviceItemProps) {
    ...
    return (
        <div className={`${className} w-full`} id={idName}>
            <Button className={"block w-full py-8 [&_svg]:fill-white"} onClick={props.onSelect}>
                <span className={"inline-block fill-white"}>
                    <PushNotificationIcon width={32} height={32} />
                </span>
                <span className={"block"}>{props.device.name}</span>
            </Button>
        </div>
    );
};
