import { Fingerprint, Info, KeyRound, LogOut, Settings, Smartphone, UserRoundCog } from "lucide-react";
...
const AuthenticatedProfile = function (props: Props) {
    ...
    return (
        <section id="authenticated-profile" className="py-10 text-left">
            <div className={cn("container max-w-5xl tracking-tight", props.className)}>
                <div className="flex flex-col gap-3 border-b pb-8">
                    ...
                    <div className="flex w-full items-center gap-2 rounded-lg bg-muted px-4 py-3 text-sm font-medium text-muted-foreground">
                        ...
                        {translate(
                            "You are signed in. Manage your account and sign-in methods from the settings area.",
                        )}
                    </div>
                </div>
                ...
                <div className="mt-10 flex flex-col items-start gap-8 sm:mt-0 sm:flex-row">
                    <div className="flex flex-col items-start gap-4 sm:py-10">
                        ...
                        <div className="space-y-2">
                            <p className="text-lg font-semibold">{userInfo.display_name}</p>
                            {primaryEmail ? <p className="text-sm text-muted-foreground">{primaryEmail}</p> : null}
                            <div className="flex items-center gap-3 pt-1">
                                <Button id="profile-settings" type="button" onClick={() => navigate(SettingsRoute)}>
                                ...
                                {flowPresent ? (
                                    ...
                                    >
                                        <UserRoundCog /> {translate("Switch User")}
                                    </Button>
                                ) : null}
                            </div>
                            <Button
                    ...
                    <div className="w-full space-y-8 sm:border-l sm:py-10 sm:pl-8">
                        <div className="grid gap-4 gap-y-8 border-b pb-8">
                            <div className="space-y-2">
                                ...
                                <Input id="display-name" defaultValue={userInfo.display_name} disabled />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="email">{translate("Primary Email Address")}</Label>
                                <div>
                                    <Input id="email" defaultValue={primaryEmail ?? ""} disabled />
                        ...
                        <div className="grid gap-4">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    defaultValue={methodLabel(userInfo.method, translate)}
                                    disabled
                                />
                            </div>
